...
locals {
  ...
  # Jaeger prefixes (jaeger-span, jaeger-service) need special handling:
  # their priority-200 templates must include fielddata=true on text fields or
  # ES 8.x will refuse sort/aggregation queries with "all shards failed".
  custom_template_commands = join("\n\n", [
    for prefix, days in var.custom_ilm_policies : (
      startswith(prefix, "jaeger-") ?
      <<-EOT
      # Jaeger ILM template — priority 200, ILM settings + fielddata for ES 8.x
      curl -sk -u "elastic:$PASSWORD" -X PUT \
      ...
      spec {
        ...
        container {
          ...
          args = [<<-EOF
            ...
            # Jaeger fielddata override templates (priority 100 — backup/legacy).
            # The primary fix lives in the telemetry-jaeger-*-ilm templates above
            # (priority 200, generated by custom_template_commands), which win and
            # include fielddata:true. These priority-100 templates are kept as a
            # safety net for any non-jaeger-* prefixed patterns that might match.
            curl -sk -u "elastic:$PASSWORD" -X PUT \
